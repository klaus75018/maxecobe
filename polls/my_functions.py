...
def sql_to_json_P1(parents,childs):
     ...
     for myParent1 in parents.objects.all():
          ...
          if type(getattr(myParent1,re.sub("polls."+parentstable+".","",str(parentInfolist[0][1])))) != str:
               parentsAndChilds[nb_de_parents].update({"parent_id": getattr(myParent1,re.sub("polls."+parentstable+".","",str(parentInfolist[0][1]))).id})
     ...
     for myParent in parentsAndChilds:
          ...
          for mychild in childs.objects.all():
               ...
               if getattr(mychild,re.sub("polls."+childstable+".","",str(childInfolist[0][1]))).id != None and  myParent["id"] != None and float(myParent["id"]) == float(getattr(mychild,re.sub("polls."+childstable+".","",str(childInfolist[0][1]))).id):
                    ...
                    mychildinfos["id"] = mychild.id
                    mychildinfos["parent_id"] = getattr(mychild,re.sub("polls."+childstable+".","",str(childInfolist[0][1]))).id
...
from .my_functions import cleaningids_parentids, sql_to_json_P1, cleaningids_parentids_initiation, mergesuccessingjsons, mergetwinsjsons
import re
import pandas as pd
import json
...
def cleaningids_parentids_initiation(jsonbrut):
     
     import json
     mylist_to_clean = json.loads(jsonbrut)
     myvar_dic = {}
     myvar_dic2 = {}
     myvar_dic3 = {}
     ...
     cleaned_list = cleaningids_parentids(myi,myk,myv,myvar_dic,myvar_dic2,myvar_dic3,mylist_to_clean)
     return cleaned_list
 
     

def cleaningids_parentids(i,k,v,var_dic,var_dic2,var_dic3,list_to_clean):
     var_dic.append({i.copy():ii})
     var_dic2.append({k.copy():kk})
     var_dic3.append({v.copy():vv})
     mycleaned_list = []

     for entry in list_to_clean:
          for var_dic[i] in range(0,len(entry)):
               
               for var_dic2[k], var_dic3[v] in entry[var_dic[i]]:
                    if var_dic2[k] == "id":
                         del entry[var_dic[i]][var_dic2]
                         
                    if var_dic2[k] == "parent_id":
                         del [var_dic[i]][var_dic2]
                    if type(var_dic3[v]) == list:
                         i+=1
                         k+=1
                         v+=1
          
                         var_dic3[v] = cleaningids_parentids(i,k,v,var_dic,var_dic2,vv)
          mycleaned_list.clear
          for var_dic[i] in range(0,len(entry)):
              for var_dic2[k], var_dic3[v] in entry[var_dic[i]]:
                 mycleaned_list.append({var_dic2[k]:var_dic3[v]})   

          cleaningids_parentids()
...
#c'est la qu'il faut que je m'assurer que les Twin ont exactement le meme nombre de ligne dans le même order
    ...
    # print("RESULT ")
     ...
     parsed = json.loads(myjson)
     with open("final.json",'w') as fi:
          json.dump(parsed, fi, indent=4)

     parsed2=cleaningids_parentids_initiation(myjson)
     with open("finalpropre.json",'w') as fi2:
          json.dump(parsed2, fi2, indent=4)